...
1. Отбирает финалистов из оцененного пула. Отбор детерминированный: сортировка
   ...
   отдельный жесткий потолок для препринтов arXiv — без него AWS с его
   одиннадцатью статьями про собственный AgentCore занял бы четверть выпуска.

2. Пишет для отобранных анонс — одно-два предложения, по которым читатель
   решает, открывать или нет. Материалов здесь пара десятков, а не сотни.
   Выводы, ожидаемый результат и ограничения писались до 2026-09-09 и больше
   не пишутся: на карточке их нет, а платить за невидимый текст незачем.

Правило про запрещенную на сайте букву не доверяется модели: запрет стоит в
промте И механически применяется к результату. Мелкое механическое правило
нельзя оставлять на усмотрение — kb/check.py все равно остановит публикацию, а
...
# Цена держится рядом с моделью, иначе отчет начинает врать при смене одной
# из них: после перехода с Opus на Sonnet стоимость еще считалась по Opus.
PRICE_IN, PRICE_OUT = 2.0, 10.0    # долларов за миллион токенов, Sonnet 5
BATCH_SIZE = 5
DEFAULT_VIDEOS = 10
DEFAULT_WEB = 8
...
Больше ничего писать не нужно. Ни выводов, ни ожидаемого результата, ни ограничений: на карточке стоит только анонс.
...
[{"n": 1, "анонс": "...", "уверенность": "высокая|низкая"}]"""
...
# Сама запрещенная буква записана кодом символа, а не литералом: иначе этот
# файл нарушал бы правило, которое проверяет и применяет. Тот же прием
# использован в kb/check.py.
YO_LOWER, YO_UPPER = chr(1105), chr(1025)


def no_yo(text):
    """Механическая замена запрещенной буквы. Модели тут не доверяем принципиально."""
    if isinstance(text, list):
        return [no_yo(part) for part in text]
    if not isinstance(text, str):
        return text
    return text.replace(YO_LOWER, "е").replace(YO_UPPER, "Е")
...
def on_cooldown(item, state, now):
    ...
    других, значит чаще выигрывает, значит выигрывает еще чаще.
    """
