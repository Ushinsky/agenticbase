...
def texts_of(item):
    """На карточке стоит только анонс, поэтому больше ничего и не переносим.

    Выводы, ожидаемый результат и ограничения писались до 2026-09-09 и лежат
    в старых выпусках. Генерация их прекращена: платить самой дорогой модели
    конвейера за текст, которого никто не видит, незачем.
    """
    texts = item.get("тексты") or {}
    return {"summary_ru": texts.get("анонс", "")}
