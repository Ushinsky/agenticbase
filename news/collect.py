...
Ходит по источникам из news/sources.json и складывает все найденное в единый
...
2. Каждый источник изолирован. Упал arXiv — в отчете строка про arXiv, остальные
...
4. Сухой прогон (по умолчанию) ничего не пишет на диск — только печатает отчет.

Запуск из корня репозитория:

    python news/collect.py                    # сухой прогон, отчет в консоль
...
Ключ YouTube берется из переменной окружения YOUTUBE_API_KEY. Без него
...
class Quota:
    """Счетчик квоты YouTube с жестким потолком."""
...
def fetch(url, headers=None, attempts=3):
    ...
    На 403/404 повтор бессмысленен — отдаем ошибку сразу, пусть источник
    честно попадет в отчет как сломанный.
    """
...
class Report:
    """Отчет о прогоне: что дал каждый источник и где что упало."""
...
def isolated(report, name):
    """Обертка: любое исключение внутри источника не роняет прогон.

    При успехе возвращает список, при падении — None. Вызывающий код по None
    понимает, что строку в отчет уже добавил сам обработчик ошибки, и второй
    раз не пишет.
    """
    def wrapper(function, *args, **kwargs):
        try:
            return function(*args, **kwargs)
        except Exception as error:  # noqa: BLE001 — тут ловим намеренно все
...
def youtube_error_reason(error):
    """Достает машинную причину отказа из тела ответа Google.

    Google кладет в 403 поле reason: quotaExceeded, accessNotConfigured,
    ipRefererBlocked и так далее. Различать их обязательно — иначе невключенный
    в проекте API выглядит как исчерпанный лимит, и отчет врет.
    """
    try:
...
def youtube_search(api_key, config, quota, since):
    ...
    for query in queries:
        ...
        try:
            ...
            # останавливаемся, это штатный конец рабочего дня. Все остальное
            # (API не включен в проекте, ключ ограничен по адресу) — настоящая
            # поломка, и она должна попасть в отчет своими словами, а не
...
def collect_hackernews(config, since):
    """Algolia API — открытый, без ключа. Берем истории выше порога очков."""
    results = []
    seen = set()
...
def collect_reddit(config, since):
    ...
    User-Agent, RSS отдается свободно. Плата за это — из ленты не приходит счет
    голосов, поэтому порога по очкам нет: выдача /top за неделю уже отсортирована
    по популярности, берем первые limit записей.

    В теле записи Reddit кладет ссылку на внешний материал. Если она есть —
    кандидатом становится сам материал, а обсуждение остается справочной ссылкой.
    """
    namespace = {"atom": "http://www.w3.org/2005/Atom"}
    results = []
...
def main():
    ...
    def gather(label, function, *args, note=""):
        """Один источник: выполнить, забрать результат, вписать строку в отчет."""
