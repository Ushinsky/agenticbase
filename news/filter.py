...
её нельзя отдавать модели. Модель на вопросе «PT2M47S — это меньше трёх
минут?» почти всегда права, а код прав всегда.

Всё, что отсеяно, попадает в журнал с причиной — чтобы на вопрос «почему
...
MIN_DURATION_SEC = 180          # ролики короче трёх минут не берём
...
def check(item, state, now):
    ...
    Порядок проверок — от самой дешёвой и бесспорной к более тонкой.
    """
