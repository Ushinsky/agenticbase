...
Считает баллы всем, кто прошёл классификацию. Только цифры, без прозы:
анонсы, выводы и ограничения пишет следующий шаг (write.py) и только для
финалистов. Считать баллы и писать тексты — разные задачи, и платить за
вторую там, где нужна первая, незачем.
...
ЯКОРЬ 90 — "How Figma Uses AI Agents for Security" (InfoQ): названная компания рассказывает, как устроена ее собственная система в продакшене, с механикой и ограничениями. Опыт из первых рук, воспроизводимый подход.
...
def popularity_percentiles(items):
    ...
    модели не должны прижимать к нулю всё остальное. Скорость набора
...
def run(client, taste, items):
    ...
    for start in range(0, len(items), BATCH_SIZE):
        ...
        if result is None:
            # Не оценили — материал не выбрасываем, но и в финал он не пройдёт:
