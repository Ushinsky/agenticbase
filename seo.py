...
EXTRA = {
    ...
    "news/archive/index.html": (
        "Архив дайджестов",
        "Прошлые дайджесты еженедельной ленты материалов про ИИ-агентов.",
        "CollectionPage",
    ),
    "hub/index.html": (
        "Хаб агентов",
...
# «Дайджест #7 — 5-11 сентября — Запуск ИИ-агентов», из него и берем период.
FAMILIES = (
    (
        re.compile(r"^news/weeks/[^/]+\.html$"),
        re.compile(r"^(Дайджест\s+#\d+)\s+—\s+([^—]+?)\s+—"),
        "Архивный дайджест ленты за %s: ролики и статьи про ИИ-агентов, "
        "отобранные и коротко пересказанные. Материалы дайджеста больше не меняются.",
        "CollectionPage",
    ),
)
