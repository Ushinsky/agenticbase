...
запрещённого слова, который в прошлом выпуске получил оценку 92.
...
2. Сбой трактуется как пропуск, а не как отказ. Не разобрался ответ, упал API,
   кончился лимит — кандидат идёт дальше. Фильтр, который при поломке молча
   всё отсеивает, — это ровно тот сценарий, который дал три материала в выпуске.
...
По каждому материалу верни решение. Отклоняй только при уверенности: если сомневаешься — пропускай дальше, следующий ярус разберётся. Ошибочно пропущенный материал стоит дешево, ошибочно выброшенный теряется навсегда.
...
def parse_answer(text, size):
    """Достаёт JSON-массив из ответа. None, если разобрать не удалось."""
...
def main():
    ...
    if share > MAX_REJECT_SHARE:
        print("ОСТАНОВКА: отклонено %.0f%% пула. Это признак сломавшегося промта,"
              " а не плохой недели. Результат не сохранён." % (share * 100))
        return 2
